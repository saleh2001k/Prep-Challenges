...
const recursionPattern = (int1, int2) => {
  ...
  const recursionHelper = (num, diff, add) => {
    if (num <= 0) {
      add = true;
    }
    ...
    if (add) {
      pattern.push(num + diff);
      return recursionHelper(num + diff, diff, add);
    } else {
      pattern.push(num - diff);
      return recursionHelper(num - diff, diff, add);
    }
  ...
  recursionHelper(int1, int2, false);
  return pattern;
};
...
const filterLinks = (str) => {
  const regex = /href=["'](?:https?:\/\/)?([^\s"'<>]+).(com|org|net)["']/;
