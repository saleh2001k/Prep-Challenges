...
const objLat = (obj) => {
    ...
    const firstNameToUpperCase = firstName.charAt(0).toUpperCase() + firstName.slice(1);
    const lastNameToUpperCase = lastName.charAt(0).toUpperCase() + lastName.slice(1);
    return `my name is ${firstNameToUpperCase} ${lastNameToUpperCase} I am ${age} YO, and I love ${hobby}.`;
  };
// -------------------------------------------------------------------------------------------------------
...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (!fullName || cv.yearsOfExperience <= 1) {
      ...
      switch (cv.tech) {
        ...
        default: 
        console.log('We take only (js, .net, python, java).');
...
const classesAvg = (data) => {
    for (let i = 0; i < data.grades.length; i++) {
      const studentsGrades = data.grades[i];
      
      for (let j = 0; j < studentsGrades.classes.length; j++) {
        const classData = studentsGrades.classes[j];
        let sum = 0;
        
            for (let k = 0; k < classData.classSum.length; k++) {
            sum += classData.classSum[k];
            }

        const avg = Math.floor(sum / classData.classSum.length);
