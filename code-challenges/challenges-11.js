...
function gradesAvg(arr) {
    return arr.map((student) => {
      const sum = student.gradsList.reduce((acc, curr) => acc + curr, 0);
      const avg = sum / student.gradsList.length;
      const newStudent  = Object.assign({}, student, { avg });
      return newStudent;
    });
  }  
...
function studentsResult(arr) {
    return arr.map((student) => {
        ...
        const avg = sum / student.gradsList.length;
        const newStudent  = Object.assign({}, student, { avg });
        if (avg >= 50) {newStudent.result = 'Passed';}
        else {newStudent.result = 'Failed';}
        return newStudent;
      });
}
// -------------------------------------------------------------------------------------------------------
